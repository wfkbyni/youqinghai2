...
#define VOLog( s, ... ) \
do{\
    if([VODBCache sharedCache].logsErrors){  \
...
- (void)cacheObjectUpdateWhenConflict:(BOOL)update
    ...
    [[VODBCache sharedCache].cacheQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        db.logsErrors = [VODBCache sharedCache].logsErrors;
        // 3.生成SQL语句
            ...
            if ([obj isKindOfClass:[self class]]) {
                ...
                if (cacheValues) {
                    ...
                    NSString *sql = [NSString stringWithFormat:@"update \"%@\" SET %@ WHERE \"%@\" = \"%@\"",
                                     tableName,cacheValues, VODBConstraint,[obj uniquenessConstraint]];
                    // 4.执行SQL语句
                    [db executeUpdate:sql];
                }
            }
...
+ (void)objectsFromCacheWithSQL:(NSString *)sql
    ...
    [[VODBCache sharedCache].cacheQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        while ([rs next]) {
            id obj = [[self class] mj_objectWithKeyValues:rs.resultDictionary];
            if (obj) {
...
+ (NSString *)fieldSQL:(MJProperty *)property{
    ...
    if ([self respondsToSelector:@selector(mj_replacedKeyFromPropertyName)]) {
        NSDictionary *dic = [self performSelector:@selector(mj_replacedKeyFromPropertyName)];
        if (dic[name]) {
            name = dic[name];
        }
    }
...
+ (NSArray *)fieldsInTable:(NSString *)table inDataBase:(FMDatabase *)db{
    ...
    FMResultSet *rs = [db executeQuery:sql];
    while ([rs next]) {
        VOSqliteField *field = [VOSqliteField mj_objectWithKeyValues:rs.resultDictionary];
        if (field) {
