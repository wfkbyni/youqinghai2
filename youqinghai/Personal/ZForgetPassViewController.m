...
@end
@implementation ForgModel



@end
@interface ZForgetPassViewController ()
{
    NSInteger seconds;
}
@property(nonatomic,strong)NSMutableArray *cellar;
@property(strong,nonatomic)NSTimer *timer;
@property(strong,nonatomic)ForgModel *reqModel;
@end

@implementation ZForgetPassViewController
...
- (void)viewDidLoad {
    ...
    _reqModel = [[ForgModel alloc]init];
...
-(UIView*)footView
{
    ...
    [button setTitle:@"修改并登录" forState:UIControlStateNormal];
...
//            }
    ...
    RACSignal *signal = [[RequestBaseAPI standardAPI] userForgetWithPhone:_reqModel.phone withPassWord:_reqModel.password withsmscode:_reqModel.serverCod withtype:1];
    __weak ZForgetPassViewController *BlockSelf = self;
    [signal subscribeNext:^(id x) {
        NSLog(@"%@",x);
        
        RACSignal *signal = [[ZUserModel shareUserModel]getUserDataWithPhone:BlockSelf.reqModel.phone andPassWord:BlockSelf.reqModel.password];
        [signal subscribeNext:^(id x) {
...
-(void)getCode:(UIButton*)btn
{
    ...
    RACSignal *signal = [[RequestBaseAPI standardAPI]phoneWithphone:cell.fieldText.text  withtype:@"1" withState:@"0"];
    [signal subscribeNext:^(id x) {
