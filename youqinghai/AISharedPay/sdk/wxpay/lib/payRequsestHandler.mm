...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPay_Weixin:(NSDictionary *)dict
{

    //订单标题，展示给用户
    NSString *order_name    =  [dict objectForKey:@"order_name"];//@"V3支付测试";
    //订单金额,单位（分）
    NSString *order_price   =  [dict objectForKey:@"order_price"] ; //@"1";//1分钱测试
    ...
    [packageParams setObject: @"APP-iOS"        forKey:@"device_info"]; //支付设备号或门店号
    [packageParams setObject: noncestr          forKey:@"nonce_str"];   //随机串
    [packageParams setObject: @"APP"            forKey:@"trade_type"];  //支付类型，固定为APP
    [packageParams setObject: order_name        forKey:@"body"];        //订单描述，展示给用户
  //  [packageParams setObject: WX_NOTIFY_URL        forKey:@"notify_url"];  //支付结果异步通知
